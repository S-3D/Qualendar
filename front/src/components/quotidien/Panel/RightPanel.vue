...
const { listeActivitesDuJour } = storeToRefs(useListeActivitesStore())
const selectedItemId = ref()
const dense = computed( ()=>
  listeActivitesDuJour.value.length > 3 ?
  true:
  false
)
    ...
    <q-scroll-area style="height: 100%; width: auto" :bar-style="{ background: 'transparent', }"
      ...
      <q-list >
        <q-slide-item
          ...
          @action = "e=>console.log('e', e) "
          right-color="negative"
          class="q-my-sm shadow-4"
          style="border: 1px solid rgb(173, 170, 170); border-radius: 5px;"
          >

          <ActiviteUnitaire :activite=activite :totalItems=listeActivitesDuJour.length />
